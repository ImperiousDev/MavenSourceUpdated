...
namespace Core {
	...
	BOOLEAN GetTargetHead(FVector& out) {
		...
		auto mesh = ReadPointer(Core::TargetPawn, 0x280);//We need to find the names for this
		if (!mesh) {
			return FALSE;
		}

		auto bones = ReadPointer(mesh, 0x478);
		if (!bones) bones = ReadPointer(mesh, 0x478 + 0x10);//We need to find the names for this
		if (!bones) {
			return FALSE;
		}
