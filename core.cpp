...
namespace Core {
	...
	BOOLEAN Initialize() {
		...
		// CalculateSpread
		addr = Util::FindPattern("\x83\x79\x78\x00\x4C\x8B\xC9\x75\x0F\x0F\x57\xC0\xC7\x02\x00\x00\x00\x00\xF3\x41\x0F\x11\x00\xC3\x48\x8B\x41\x70\x8B\x48\x04\x89\x0A\x49\x63\x41\x78\x48\x6B\xC8\x1C\x49\x8B\x41\x70\xF3\x0F\x10\x44\x01\x00\xF3\x41\x0F\x11\x00\xC3", "xxx?xxxxxxxxxx????xxxx?xxxxxxxxxxxxxxxxxxxxxxxxxxx?xxxx?x"); 
		if (!addr) {
		...
		addr = Util::FindPattern("\x0F\x57\xD2\x48\x8D\x4C\x24\x00\x41\x0F\x28\x00\xE8\x00\x00\x00\x00", "xxxxxxx?xxx?x????"); 
		if (!addr) {
		...
		//CalculateShot
		addr = Util::FindPattern("", "xxxxxxx????xxxxx????xx?????xxxxx????xxxxx????xxx????xxxx?xxx????xxx");
		if (!addr) {
		...
		// Reload
		Util::FindPattern("", "xxxxxxxxx?xxx????xxxx????xxxxxxxxxxxxxxxxx????xxxxx????xxxx????xxxxx????xxxxxxx");
		if (!addr) {
